...
def encode(pkm):
  s=list(struct.unpack("IHH"+"H"*(old_div(len(pkm),2)-4), pkm))
  ...
  return struct.pack("IHH"+"H"*(old_div(len(pkm),2)-4), *shifted)


def decode(bin):
  shifted=list(struct.unpack("IHH"+"H"*(old_div(len(bin),2)-4), bin))
  ...
  s+=shifted[67:]
  return struct.pack("IHH"+"H"*(old_div(len(bin),2)-4), *s)
